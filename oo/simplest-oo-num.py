...
class Dense:
	def __init__(self, weights=[10,10]):
		self.type = 'Dense'
		self.weights = np.random.randn(weights[0], weights[1]) * np.sqrt(2.0/weights[0])
...
class Softplus:
	...
	def forward(self, z):
		self.input = z
		self.output = np.log(1.0 + np.exp(z))
		return self.output

	def backward(self, grad, lr):
		ez = np.exp(self.input)
...
class Softmax:
	...
	def forward(self, z):
		self.input = z
		ez = np.exp(z)
		self.output = ez / np.sum(ez, axis=1).reshape(z.shape[0], 1)
		return self.output
