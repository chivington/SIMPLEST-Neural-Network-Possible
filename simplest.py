...
def softmax(Z):
	ez = cu.exp(Z)
	return ez / cu.sum(ez, axis=1).reshape(ez.shape[0], 1)

def init_weights(layers):
	print(f'\n Initializing network...')
	weights = []
	for l, layer in enumerate(layers[:-1]):
		weights.append(cu.random.randn(layer, layers[l+1]) * cu.sqrt(2.0/layer))
...
def train(trn_x, trn_y, layers, cycles, lr, bs):
	...
	for c in range(cycles):
		...
		for b, (x,y) in enumerate(batches):
			...
			backward(error, weights, forward_pass, lr)
